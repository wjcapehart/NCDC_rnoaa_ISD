...
ISD Parsing is dfone with the "isdparser"

You will need to install all three

I also included "openair" for wind roses

Something to be aware of before we start.  

These are an merger of several report message types.  Most are derived for use at airports. The data archiving is designed to house any of the messages in one single record.
...
If you didn't work in the old Asheville Federal Building in the 80s before them moved and don't know what file cabinet to use there is hope.
...
use the isd command following this example.  The orignal data is kept in a compressed "tarball" with one tarball per year.
...
temp/dewpoint (deg C) converted from 10ths of degree
...
Precip is also processed but this data set is an amalgum of both hourly, 3-hrly, 6-hrly, 12-hrly, 24-hrly data depending on the report message that's being archived and I am not sure you will want that, especially if you can get Hourly Precip Data from another source
...
I am setting it up to give you the "GF1 group" which is the total cloud cover for all flight levels.  These fields are often archived in "octaves" or eights and I'll be converting them to fractions for you.  IN cases of "obscured sky" caused by fog or smoke, I will label it 100% for total obscuration or 50% covered for partial obscuration.    
...
Finally let's put everything into a single time frame since we have observations at several times.  We can interopolate the data to the nearest hour.
